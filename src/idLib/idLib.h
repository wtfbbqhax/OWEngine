...
public:
    ...
    idFlags&        operator=( const idFlags& src )
    {
        flag = ( T )src.flag;
        return *this;
    }
    idFlags&        operator=( const T& val )
    {
        flag = val;
        return *this;
    }
    ...
    void            set( IndexType index, bool on )
    {
        if( on )
        {
            flag |= ( 1 << ( unsigned int )( index ) );
    ...
    void            setMask( const T& mask )
    {
        flag |= mask;
    }
    void            setVal( const T& f )
    {
        flag = f;
    }
    ...
    {
        return ( bool )( ( flag & ( 1 << ( unsigned int )( index ) ) ) != 0 );
    }
    bool        testMask( const T& mask ) const
    {
        return flag & mask;
    }
    bool        testMaskAll( const T& mask ) const
    {
        return ( flag & mask ) == mask;
    }
    void            toggle( IndexType index )
    {
        flag ^= ( 1 << ( unsigned int )( index ) );
    }
    ...
    unsigned int    getNumBits( void ) const
    {
        return sizeof( T ) * 8;
    }
    
    static const char* getDebugNames()
    {
        return debugNames;
    }
private:
