...
int ( *qglXGetVideoSyncSGI )( unsigned int* count );
int ( *qglXWaitVideoSyncSGI )( int divisor, int remainder, unsigned int* count );
const char* ( *qglXQueryExtensionsString )( Display* dpy, int screen );
...
static void* glGetProcAddress( char* symbol )
{
    if( glw_state.OpenGLLib )
    {
        return GPA( symbol );
