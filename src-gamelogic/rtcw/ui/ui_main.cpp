...
static void UI_DrawTeamName( rectDef_t* rect, int font, float scale, vec4_t color, bool blue, int textStyle )
{
    int i;
...
static void UI_DrawTeamMember( rectDef_t* rect, int font, float scale, vec4_t color, bool blue, int num, int textStyle )
{
#ifdef MISSIONPACK
    // 0 - None
    // 1 - Human
...
static void UI_DrawPlayerModel( rectDef_t* rect )
{
    ...
    //   UI_PlayerInfo_SetInfo( &info, LEGS_IDLE, TORSO_STAND, viewangles, moveangles, WP_MP40, false );
...
#endif  // #ifdef MISSIONPACK
        UI_PlayerInfo_SetInfo( &info2, LEGS_IDLE, TORSO_STAND, viewangles, vec3_origin, WP_MP40, false );
#ifdef MISSIONPACK
        UI_RegisterClientModelname( &info2, model, headmodel, team );
#else
        UI_RegisterClientModelname( &info2, model );
#endif  // #ifdef MISSIONPACK
        updateOpponentModel = false;
    }
...
static void UI_DrawAllMapsSelection( rectDef_t* rect, int font, float scale, vec4_t color, int textStyle, bool net )
{
#ifdef MISSIONPACK
    int map = ( net ) ? ui_currentNetMap.integer : ui_currentMap.integer;
    if( map >= 0 && map < uiInfo.mapCount )
...
//----(SA)	end
        ...
        case UI_SKILL:
            ...
            break;
        case UI_BLUETEAMNAME:
            UI_DrawTeamName( &rect, font, scale, color, true, textStyle );
            break;
        case UI_REDTEAMNAME:
            UI_DrawTeamName( &rect, font, scale, color, false, textStyle );
            break;
        case UI_BLUETEAM1:
        case UI_BLUETEAM2:
        case UI_BLUETEAM3:
        case UI_BLUETEAM4:
        case UI_BLUETEAM5:
            UI_DrawTeamMember( &rect, font, scale, color, true, ownerDraw - UI_BLUETEAM1 + 1, textStyle );
            break;
        case UI_REDTEAM1:
        case UI_REDTEAM2:
        case UI_REDTEAM3:
        case UI_REDTEAM4:
        case UI_REDTEAM5:
            UI_DrawTeamMember( &rect, font, scale, color, false, ownerDraw - UI_REDTEAM1 + 1, textStyle );
            break;
        case UI_NETSOURCE:
        ...
        case UI_TIER_GAMETYPE:
            ...
            break;
        case UI_ALLMAPS_SELECTION:
            UI_DrawAllMapsSelection( &rect, font, scale, color, textStyle, true );
            break;
        case UI_MAPS_SELECTION:
            UI_DrawAllMapsSelection( &rect, font, scale, color, textStyle, false );
            break;
        case UI_OPPONENT_NAME:
...
void WM_setWeaponPics()
{
    ...
    if( playerType == 0 )
    {
        WM_setVisibility( "window_feeder_soldierweap", true );
    }
    else
    {
        WM_setVisibility( "window_feeder_soldierweap", false );
    }
    
    if( playerType == 3 )
    {
        WM_setVisibility( "window_feeder_lieutweap", true );
    }
    else
    {
        WM_setVisibility( "window_feeder_lieutweap", false );
    }
    
    // don't allow spectators to cycle through menus
    if( team == 2 )
    {
        WM_setVisibility( "window_pickTeamNext", false );
        WM_setVisibility( "window_pickTeamNextCmd", false );
        WM_setVisibility( "window_pickTeamNextDisabled", true );
    ...
    {
        WM_setVisibility( "window_pickTeamNext", true );
        WM_setVisibility( "window_pickTeamNextCmd", true );
        WM_setVisibility( "window_pickTeamNextDisabled", false );
    }
}
...
void WM_HideItems()
{
    ...
    Menu_ShowItemByName( menu, "window_pickplayer", false );
    Menu_ShowItemByName( menu, "window_weap", false );
    Menu_ShowItemByName( menu, "weap_*", false );
    Menu_ShowItemByName( menu, "pistol_*", false );
    Menu_ShowItemByName( menu, "grenade_*", false );
    Menu_ShowItemByName( menu, "player_type", false );
}
    ...
    if( Menu_Count() > 0 )
    {
        ...
        switch( menu )
        {
            ...
            case UIMENU_MAIN:
                //trap_Cvar_Set( "sv_killserver", "1" );
                trap_Key_SetCatcher( KEYCATCH_UI );
                //trap_S_StartLocalSound( trap_S_RegisterSound("sound/misc/menu_background.wav", false) , CHAN_LOCAL_SOUND );
