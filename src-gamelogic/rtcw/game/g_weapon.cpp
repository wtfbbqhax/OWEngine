...
void Weapon_Syringe( gentity_t* ent )
{
    ...
    bool usedSyringe = false;          // DHM - Nerve
    ...
    if( tr.fraction < 1.0 )
    {
        ...
        if( traceEnt->client != NULL )
        {
            if( ( traceEnt->client->ps.pm_type == PM_DEAD ) && ( traceEnt->client->sess.sessionTeam == ent->client->sess.sessionTeam ) )
            {
                ...
                {
                    BG_AnimScriptEvent( &traceEnt->client->ps, ANIM_ET_JUMP, false, true );
                }
                else
                {
                    BG_AnimScriptEvent( &traceEnt->client->ps, ANIM_ET_REVIVE, false, true );
...
//	int			mod = MOD_KNIFE;
    ...
    if( traceEnt->methodOfDeath == MOD_DYNAMITE )
    {
        ...
        if( traceEnt->health >= 248 )
        {
            ...
            Add_Ammo( ent, WP_DYNAMITE, 1, true );
    ...
    else if( !traceEnt->takedamage && !Q_stricmp( traceEnt->classname, "misc_mg42" ) )
    {
        ...
        if( traceEnt->health >= 255 )
        {
            ...
            if( traceEnt->mg42BaseEnt > 0 )
            {
                ...
                g_entities[ traceEnt->mg42BaseEnt ].takedamage = true;
                traceEnt->health = 0;
            }
            else
            {
    ...
    if( g_gametype.integer == GT_SINGLE_PLAYER )
    {
        switch( weapon )
        {
            ...
            case WP_AKIMBO:
                return 8;       //----(SA)	added
            case WP_VENOM:
                return 12;       // 15  ----(SA)	slight modify for DM
            case WP_MP40:
                return 6;
            case WP_THOMPSON:
                return 8;
...
void VenomPattern( vec3_t origin, vec3_t origin2, int seed, gentity_t* ent )
{
    ...
    for( i = 0 ; i < DEFAULT_VENOM_COUNT ; i++ )
    {
        ...
        VectorMA( origin, 8192, _forward, end );
        VectorMA( end, r, _right, end );
        VectorMA( end, u, _up, end );
...
void AddLean( gentity_t* ent, vec3_t point )
{
    if( ent->client )
    {
        ...
        {
            vec3_t _right;
            AngleVectors( ent->client->ps.viewangles, NULL, _right, NULL );
            VectorMA( point, ent->client->ps.leanf, _right, point );
        }
    }
}
...
===============
*/
void CalcMuzzlePoint( gentity_t* ent, int weapon, vec3_t _forward, vec3_t _right, vec3_t _up, vec3_t muzzlePoint )
{
...
//	VectorMA( muzzlePoint, 14, forward, muzzlePoint );
    ...
    {
        case WP_PANZERFAUST:
//			VectorMA( muzzlePoint, 14, _right, muzzlePoint );	//----(SA)	new first person rl position
            VectorMA( muzzlePoint, 10, _right, muzzlePoint );        //----(SA)	new first person rl position
            VectorMA( muzzlePoint, -10, _up, muzzlePoint );
            break;
...
//			break;
        ...
        case WP_GRENADE_PINEAPPLE:
        case WP_GRENADE_LAUNCHER:
            VectorMA( muzzlePoint, 20, _right, muzzlePoint );
            break;
        case WP_AKIMBO:     // left side rather than right
            VectorMA( muzzlePoint, -6, _right, muzzlePoint );
            VectorMA( muzzlePoint, -4, _up, muzzlePoint );
        default:
            VectorMA( muzzlePoint, 6, _right, muzzlePoint );
            VectorMA( muzzlePoint, -4, _up, muzzlePoint );
            break;
...
// Rafael - for activate
void CalcMuzzlePointForActivate( gentity_t* ent, vec3_t _forward, vec3_t _right, vec3_t _up, vec3_t muzzlePoint )
{
...
//----(SA)	end
        ...
        case WP_TESLA:
            ...
            if( !ent->aiCharacter )
            {
                vec3_t _forward, vangle;
                VectorCopy( ent->client->ps.viewangles, vangle );
                vangle[PITCH] = 0;  // nullify pitch so you can't lightning jump
                AngleVectors( vangle, _forward, NULL, NULL );
                // make it less if in the air
                if( ent->s.groundEntityNum == ENTITYNUM_NONE )
                {
                    VectorMA( ent->client->ps.velocity, -32, _forward, ent->client->ps.velocity );
                }
                else
                {
                    VectorMA( ent->client->ps.velocity, -100, _forward, ent->client->ps.velocity );
                }
            }
            break;
        case WP_GAUNTLET:
