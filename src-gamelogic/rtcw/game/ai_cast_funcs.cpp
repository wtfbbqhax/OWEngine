...
char* AIFunc_BattleAmbush( cast_state_t* cs )
{
    ...
    vec3_t destorg, vec;
    float dist, moveDist;
    int _enemies[MAX_CLIENTS], _numEnemies, i;
    ...
    else if(_numEnemies > 0 )
    {
        if( AICast_GotEnoughAmmoForWeapon( cs, cs->weaponNum ) )
        {
            ...
            for( i = 0; i < _numEnemies; i++ )
            {
                ...
                if( ( AICast_EntityVisible( cs, _enemies[i], false ) && AICast_CheckAttack( cs, _enemies[i], false ) ) ||
                ...
                {
                    cs->lastEnemy = _enemies[i];
                }
                else if( AICast_EntityVisible( cs, _enemies[i], false ) )
                {
                    bot_input_t bi_back;
                    // try and move to them, if successful, then start chasing
                    trap_EA_GetInput( cs->entityNum, ( float ) level.time / 1000, &bi_back );
                    if( AICast_MoveToPos( cs, g_entities[_enemies[i]].client->ps.origin, _enemies[i] ) )
                    {
...
char* AIFunc_BattleTakeCover( cast_state_t* cs )
{
    ...
    else if(_numEnemies == -2 )          // inspection may be required
    {
        char* retval;
        // TTimo: gcc: suggest () around assignment used as truth value
        if( ( retval = AIFunc_InspectFriendlyStart( cs, _enemies[0] ) ) )
        {
            return retval;
        }
    }
...
// DISABLED: doesn't work well with new AI system
    ...
    if( VectorLength( cs->takeCoverPos ) > 1 && dist > 8 )
    {
        ...
        // do we want to play a rolling animation into a cover position?
                 ...
                 ( shouldAttack && !AICast_VisibleFromPos( g_entities[cs->enemyNum].s.pos.trBase, cs->enemyNum, move.endpos, cs->entityNum, false ) ) )
        {
            ...
            dist > 128 )
    {
                ...
                ( cs->bs->cur_ps.groundEntityNum != ENTITYNUM_NONE || g_entities[cs->entityNum].waterlevel > 1 ) )
        {
            ...
            if( tr.entityNum == followent->s.number )
            {
                ...
                {
                    //trap_EA_Move(cs->entityNum, _dir, 0);
                }
            }
        }
        // just go to them
...
//		if ((cs->grenadeGrabFlag == -1) || (cs->grenadeGrabFlag == true && level.time > danger->nextthink - 2000)) {	// kick
        ...
        /*
        			...
        			if (cs->grenadeGrabFlag == true) {
        				...
        				BG_AnimScriptEvent( &ent->client->ps, ANIM_ET_PICKUPGRENADE, false, true );
    ...
    /*	//
    	...
    	if (VectorDistance( danger->r.currentOrigin, cs->bs->origin ) < 128) {
    		...
    		cs->weaponNum = weapon;	// select grenade launcher
    		shouldAttack = false;	// don't attack until we've caught it
    	}
    */
    // if we should be attacking something on our way
    if( shouldAttack )
...
char* AIFunc_Battle( cast_state_t* cs )
{
    ...
    //
    /*
    _moveresult = AICast_CombatMove(cs, tfl);
    //if the movement failed
    if (_moveresult.failure) {
    	//reset the avoid reach, otherwise bot is stuck in current area
    	trap_BotResetAvoidReach(bs->ms);
    ...
    */
    ...
    //
    AICast_Blocked( cs, &_moveresult, false, NULL );
    //
    // Retreat?
