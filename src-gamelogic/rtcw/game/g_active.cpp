...
//	}
    ...
    BG_UpdateConditionValue( ent->s.number, ANIM_COND_PARACHUTE, ( ent->flags & FL_PARACHUTE ) != 0, false );
...
//		pm.noFootsteps = true;
    ...
    ent->s.animMovetype = BG_GetConditionValue( ent->s.number, ANIM_COND_MOVETYPE, true );
    ...
    if( g_gametype.integer == GT_SINGLE_PLAYER )
    {
        switch( ent->client->ps.weapon )
        {
            case WP_TESLA:          // fear the tesla
                AICast_CheckDangerousEntity( ent, DANGER_CLIENTAIM, TESLA_RANGE + 150, 0.5, 0.6, ( ent->client->buttons & BUTTON_ATTACK ? true : false ) );
                break;
            ...
            case WP_FLAMETHROWER:           // fear the flamethrower
                AICast_CheckDangerousEntity( ent, DANGER_CLIENTAIM | DANGER_FLAMES, FLAMETHROWER_RANGE + 150, 0.5, 0.8, ( ent->client->buttons & BUTTON_ATTACK ? true : false ) );
                break;
            case WP_VENOM:              // avoid the venom
                AICast_CheckDangerousEntity( ent, DANGER_CLIENTAIM, 4000, 0.5, 0.8, ( ent->client->buttons & BUTTON_ATTACK ? true : false ) );
                break;
            case WP_PANZERFAUST:    // just predict a rocket attack, rather than waiting for them to fire, then avoiding
                AICast_CheckDangerousEntity( ent, DANGER_CLIENTAIM, 1000, 0.5, 0.8, ( ent->client->buttons & BUTTON_ATTACK ? true : false ) );
                break;
