...
//
// -------------------------------------------------------------------------
//  File name:   ai_cast_think.c
//  Version:     v1.00
//  Created:
//  Compilers:
//  Description: Wolfenstein AI Character Thinking
// -------------------------------------------------------------------------
//  History:
//
//////////////////////////////////////////////////////////////////////////////////////
...
//	}
    ...
    if( cs->enemyNum >= 0 )
    {
        ...
        {
            BG_UpdateConditionValue( ent->s.number, ANIM_COND_ENEMY_POSITION, POSITION_INFRONT, true );
        }
        else
        {
            BG_UpdateConditionValue( ent->s.number, ANIM_COND_ENEMY_POSITION, POSITION_BEHIND, true );
        }
    }
    else
    {
        BG_UpdateConditionValue( ent->s.number, ANIM_COND_ENEMY_POSITION, POSITION_UNUSED, true );
    }
    // set defense pose
    if( ent->flags & FL_DEFENSE_CROUCH )
    {
        BG_UpdateConditionValue( ent->s.number, ANIM_COND_DEFENSE, true, false );
    }
    else
    {
        BG_UpdateConditionValue( ent->s.number, ANIM_COND_DEFENSE, false, false );
    }
    //
...
void AICast_QueryThink( cast_state_t* cs )
{
    ...
    visible = AICast_VisibleFromPos( cs->bs->origin, cs->entityNum, g_entities[cs->enemyNum].r.currentOrigin, cs->enemyNum, false );
