...
//
// -------------------------------------------------------------------------
//  File name:   ai_cast_characters.c
//  Version:     v1.00
//  Created:
//  Compilers:
//  Description:
...
#define STUNNED_THRESHOLD   30
    ...
    if( g_testPain.integer == 2 || ( cs->damageQuota > painThreshold ) )
    {
        ...
        {
            BG_UpdateConditionValue( ent->s.number, ANIM_COND_STUNNED, true, false );
        }
        // enemy weapon
        if( attacker->client )
        {
            BG_UpdateConditionValue( ent->s.number, ANIM_COND_ENEMY_WEAPON, attacker->s.weapon, true );
        }
        if( point )
        {
            // location
            BG_UpdateConditionValue( ent->s.number, ANIM_COND_IMPACT_POINT, AIChar_GetPainLocation( ent, point ), true );
        }
        else
        {
            BG_UpdateConditionValue( ent->s.number, ANIM_COND_IMPACT_POINT, 0, false );
        }
        ...
        BG_UpdateConditionValue( ent->s.number, ANIM_COND_STUNNED, 0, false );
        BG_UpdateConditionValue( ent->s.number, ANIM_COND_ENEMY_WEAPON, 0, false );
        BG_UpdateConditionValue( ent->s.number, ANIM_COND_IMPACT_POINT, 0, false );
