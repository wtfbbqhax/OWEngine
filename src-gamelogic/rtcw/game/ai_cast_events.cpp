...
//
// -------------------------------------------------------------------------
//  File name:   ai_cast_events.c
//  Version:     v1.00
//  Created:
//  Compilers:
//  Description: Wolfenstein AI Character Events
// -------------------------------------------------------------------------
//  History:
//
//////////////////////////////////////////////////////////////////////////////////////
...
void AICast_Sight( gentity_t* ent, gentity_t* other, int lastSight )
{
    ...
    if( cs->sightfunc )
    {
        ...
        if( AICast_EntityVisible( cs, other->s.number, false ) )
        {
...
void AICast_Die( gentity_t* self, gentity_t* inflictor, gentity_t* attacker, int damage, int meansOfDeath )
{
    ...
    else        // this is our first death, so set everything up
    {
        ...
        if( !( self->aiCharacter == AICHAR_ZOMBIE && cs->secondDeadTime && cs->rebirthTime ) )
        {
            ...
            // set enemy weapon
            BG_UpdateConditionValue( self->s.number, ANIM_COND_ENEMY_WEAPON, 0, false );
            if( attacker->client )
            {
                BG_UpdateConditionValue( self->s.number, ANIM_COND_ENEMY_WEAPON, inflictor->s.weapon, true );
            }
            else
            {
                BG_UpdateConditionValue( self->s.number, ANIM_COND_ENEMY_WEAPON, 0, false );
            ...
            // set enemy location
            BG_UpdateConditionValue( self->s.number, ANIM_COND_ENEMY_POSITION, 0, false );
            if( infront( self, inflictor ) )
            {
                BG_UpdateConditionValue( self->s.number, ANIM_COND_ENEMY_POSITION, POSITION_INFRONT, true );
            }
            else
            {
                BG_UpdateConditionValue( self->s.number, ANIM_COND_ENEMY_POSITION, POSITION_BEHIND, true );
            }
...
void AICast_ProcessActivate( int entNum, int activatorNum )
{
    ...
    if( cs->leaderNum == activatorNum )
    {
        ...
        AIFunc_ChaseGoalStart( cs, newent->s.number, 128, true );
