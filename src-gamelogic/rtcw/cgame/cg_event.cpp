...
static void CG_ItemPickup( int itemNum )
{
    ...
    if( bg_itemlist[itemNum].giType == IT_WEAPON )
    {
        ...
        if( cg_autoswitch.integer && cg.predictedPlayerState.weaponstate != WEAPON_RELOADING )
        {
            ...
            if( weapon != WP_SNIPERRIFLE && weapon != WP_SNOOPERSCOPE && weapon != WP_FG42SCOPE && itemid != WP_AMMO )    //----(SA)	modified
            {
...
//#define	SURF_METAL              0x1000	// clanking footsteps
                ...
                if( flags & SURF_WOOD )    // SURF_WOOD
                {
                    ...
                    {
                        trap_S_StartSound( NULL, es->number, CHAN_AUTO, cgs.media.grenadebounce[GRENBOUNCE_WOOD][1] );
                    }
                }
                else if( flags & ( SURF_METAL | SURF_ROOF | SURF_GLASS ) )      //	SURF_METAL | SURF_ROOF | SURF_GLASS
                {
                    ...
                    {
                        trap_S_StartSound( NULL, es->number, CHAN_AUTO, cgs.media.grenadebounce[GRENBOUNCE_METAL][1] );
                    }
                }
                else if( flags & ( SURF_GRASS | SURF_GRAVEL | SURF_SNOW | SURF_CARPET ) )       //SURF_GRASS | SURF_GRAVEL | SURF_SNOW | SURF_CARPET
                {
                    ...
                    {
                        trap_S_StartSound( NULL, es->number, CHAN_AUTO, cgs.media.grenadebounce[GRENBOUNCE_DIRT][1] );
                    }
                }
                else
                {
                    ...
                    {
                        trap_S_StartSound( NULL, es->number, CHAN_AUTO, cgs.media.grenadebounce[GRENBOUNCE_DEFAULT][1] );
                    }
                }
            }
            break;
...
//				CG_FireFlameChunks( cent, cent->currentState.origin, cent->lerpAngles, 0.1, false, 1 );
//				CG_FireFlameChunks( cent, cent->currentState.origin, cent->currentState.apos.trBase, 0.1, false, 1 );
//			}
//			else
