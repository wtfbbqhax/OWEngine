...
#define MAX_SAME_BUF 10
int num = 0;
//В id software определенно работают местами ленивые ребята, которые не знают/не хотят знать, что такое static.
//Что ж, я ответил на их криворукость тем же... но это исправление хотя бы работает.
//EN: Fixed undefined behaviour

const char* CG_Argv( int arg )
{
    static char fuckthatbuffer [MAX_SAME_BUF][MAX_STRING_CHARS];
    num++;
    num %= MAX_SAME_BUF;
    trap_Argv( arg, fuckthatbuffer[num], sizeof( fuckthatbuffer[num] ) );
    return fuckthatbuffer[num];
}
...
bool CG_RegisterClientSkin( clientInfo_t* ci, const char* modelName, const char* skinName );
