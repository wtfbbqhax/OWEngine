...
void CM_TracePointThroughPatchCollide( traceWork_t* tw, const struct patchCollide_s* pc )
{
    ...
    for( i = 0 ; i < pc->numPlanes ; i++, _planes++ )
    {
        ...
        d1 = DotProduct( tw->start, _planes->plane ) - _planes->plane[3] + offset;
        d2 = DotProduct( tw->end, _planes->plane ) - _planes->plane[3] + offset;
        if( d1 <= 0 )
        {
            frontFacing[i] = false;
        }
