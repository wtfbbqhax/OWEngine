...
void AAS_WriteCache( fileHandle_t fp, aas_routingcache_t* nativecache )
{
    ...
    cache = ( aas_routingcache_32_t* )GetClearedMemory( size );
...
#define MAX_HIDEAREA_LOOPS  3000
    ...
    startVisible = botimport.AICast_VisibleFromPos( enemyorigin, enemynum, origin, srcnum, false );
    //
                ...
                if( ( *aasworld ).areavisibility[nextareanum] )
                {
                    ...
                    if( !AAS_AreaVisible( enemyareanum, nextareanum ) )    // now last of all, check that this area is a safe hiding spot
                    {
                        if( ( ( *aasworld ).visCache[nextareanum] == 2 ) ||
                                ( !( *aasworld ).visCache[nextareanum] && !botimport.AICast_VisibleFromPos( enemyorigin, enemynum, ( *aasworld ).areawaypoints[nextareanum], srcnum, false ) ) )
                        {
                ...
                if( AAS_AreaVisible( enemyarea, nextareanum ) )    // now last of all, check that this area is a good attacking spot
                {
                    if( ( ( *aasworld ).visCache[nextareanum] == 2 ) ||
                            ( !( *aasworld ).visCache[nextareanum] &&
                              ...
                              botimport.AICast_CheckAttackAtPos( srcnum, enemynum, ( *aasworld ).areawaypoints[nextareanum], false, false ) ) )
                    {
                        ( *aasworld ).visCache[nextareanum] = 2;
