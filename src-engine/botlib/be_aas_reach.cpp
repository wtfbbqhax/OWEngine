...
//
// -------------------------------------------------------------------------
//  File name:   be_aas_reach.c
//  Version:     v1.00
//  Created:
//  Compilers:
//  Description: reachability calculations
// -------------------------------------------------------------------------
//  History:
//
//////////////////////////////////////////////////////////////////////////////////////
...
int AAS_Reachability_Step_Barrier_WaterJump_WalkOffLedge( int area1num, int area2num )
{
    ...
    vec3_t ground_beststart = {0};
    vec3_t ground_bestend = {0};
    vec3_t ground_bestnormal = {0};
    vec3_t water_beststart = {0};
    vec3_t water_bestend = {0};
    vec3_t water_bestnormal = {0};
...
//		if (move.stopevent & (SE_ENTERSLIME|SE_ENTERLAVA)) return false;
...
int AAS_Reachability_Ladder( int area1num, int area2num )
{
    ...
    vec3_t mid, start, end, sharededgevec, dir;
    vec3_t lowestpoint = {0};
    aas_area_t* area1, *area2;
                ...
                //if (hordist < 1.6 * (facecenter[2] - areastart[2]))
                {
                    ...
                    */
                    ...
                    AAS_PredictClientMovement( &move, -1, areastart, PRESENCE_NORMAL, true,
                                               ...
                                               SE_ENTERWATER | SE_ENTERSLIME |
                                               SE_ENTERLAVA | SE_HITGROUNDDAMAGE |
                                               SE_TOUCHJUMPPAD | SE_HITGROUNDAREA, area2num, false );
                    //if prediction time wasn't enough to fully predict the movement
                    //don't enter slime or lava and don't fall from too high
                    if( move.frames < 30 &&
                            !( move.stopevent & ( SE_ENTERSLIME | SE_ENTERLAVA | SE_HITGROUNDDAMAGE ) )
