...
//
// -------------------------------------------------------------------------
//  File name:   be_aas_routetable.c
//  Version:     v1.00
//  Created:
//  Compilers:
//  Description: Area Awareness System, Route-table defines
// -------------------------------------------------------------------------
//  History:
//
//////////////////////////////////////////////////////////////////////////////////////
...
bool AAS_RT_ReadRouteTable( fileHandle_t fp )
{
    ...
    if( doswap )
    {
        for( i = 0; i < _routetable->numParents; i++, parent++ )
        {
    ...
    if( doswap )
    {
        for( i = 0; i < _routetable->numParentChildren; i++, psi++ )
        {
            *psi = LittleShort( *psi );
        }
    }
    ...
    if( doswap )
    {
        for( i = 0; i < _routetable->numVisibleParents; i++, psi++ )
        {
            *psi = LittleShort( *psi );
        }
    }
    ...
    if( doswap )
    {
        for( i = 0; i < _routetable->numParentLinks; i++, plink++ )
        {
...
//	}
    ...
    startVisible = botimport.AICast_VisibleFromPos( destpos, destnum, srcpos, srcnum, false );
    //
...
#define     GETHIDE_MAX_CHECK_PATHS     15
            ...
            for( j = 0; j < GETHIDE_MAX_CHECK_PATHS; j++ )
            {
                ...
                {
                    // success ?
                    if( !botimport.AICast_VisibleFromPos( destpos, destnum, ( *aasworld ).areas[pathArea].center, srcnum, false ) )
                    {
                        // SUCESS !!
                ...
                else
                {
                    ...
                    if( !startVisible )    //botimport.AICast_VisibleFromPos( destpos, destnum, reachability[firstreach + route->reachable_index].end, srcnum, false )) {
                    {
                        invalidRoute = true;
                        break;
                    }
...
//		if (botimport.AICast_VisibleFromPos( destpos, destnum, (*aasworld).areas[travParent->areanum].center, srcnum, false )) {
//			continue;
//		}
        //
