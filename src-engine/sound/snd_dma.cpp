...
void S_ByteSwapRawSamples( int samples, int width, int _s_channels, short* data )
{
    int i;
...
//	}
    ...
    for( i = 0, ss = streamingSounds, _re = s_rawend, rp = s_rawpainted; i < MAX_STREAMING_SOUNDS; i++, ss++, _re++, rp++ )
    {
        if( ss->kill && ss->file )
        {
            fileHandle_t file;
...
//				memset( &s_rawsamples[i], 0, MAX_RAW_SAMPLES*sizeof(portable_samplepair_t) );
        ...
        *rp = false;
        ...
        if( *_re < soundMixAheadTime )       // RF, read a bit ahead of time to allow for talking animations
        {
            *_re = soundMixAheadTime;
        }
        
//		looped = false;

        while( *_re < soundMixAheadTime + MAX_RAW_SAMPLES )
        {
            bufferSamples = MAX_RAW_SAMPLES - ( *_re - soundMixAheadTime );
            ...
            else            // attenuate if required
            {
                ...
                {
                    int _r, l;
                    S_SpatializeOrigin( snd.entityPositions[ ss->entnum ], s_volume->value * 255.0f, &l, &_r, SOUND_RANGE_DEFAULT );
                    if( ( lvol = ( ( float )l / 255.0 ) ) > 1.0 )
                    {
                        lvol = 1.0;
                    }
                    if( ( rvol = ( ( float )_r / 255.0 ) ) > 1.0 )
                    {
                        rvol = 1.0;
                    }
                    lvol *= streamingVol;
