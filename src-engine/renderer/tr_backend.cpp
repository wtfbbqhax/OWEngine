...
void RE_StretchRaw( int x, int y, int w, int h, int cols, int rows, const byte* data, int client, bool dirty )
{
    ...
    vec2_t texcoords[4];
    vec2_t verts[4];
    unsigned short indicies[6] = { 0, 1, 2, 0, 3, 2 };
    ...
    texcoords[0][0] = 0.5f / cols;
    texcoords[0][1] = 0.5f / rows;
    texcoords[1][0] = ( cols - 0.5f ) / cols;
    texcoords[1][1] = 0.5f / rows;
    texcoords[2][0] = ( cols - 0.5f ) / cols;
    texcoords[2][1] = ( rows - 0.5f ) / rows;
    texcoords[3][0] = 0.5f / cols;
    texcoords[3][1] = ( rows - 0.5f ) / rows;
...
const void* RB_StretchPic( const void* data )
{
    ...
    tess.indexes[ numIndexes ] = ( short )( numVerts + 3 );
    tess.indexes[ numIndexes + 1 ] = ( short )( numVerts + 0 );
    tess.indexes[ numIndexes + 2 ] = ( short )( numVerts + 2 );
    tess.indexes[ numIndexes + 3 ] = ( short )( numVerts + 2 );
    tess.indexes[ numIndexes + 4 ] = ( short )( numVerts + 0 );
    tess.indexes[ numIndexes + 5 ] = ( short )( numVerts + 1 );
