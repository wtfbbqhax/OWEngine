                ...
                for( k = 0 ; k < surf->numIndices ; k += 3 )
                {
                    ...
                    if( oldNumPoints != returnedPoints )
                    {
                        ...
                        for( j = 0 ; j < ( returnedPoints - oldNumPoints ) ; j++ )
                        {
                            ...
                            *( ( float* )pointBuffer + 5 * ( oldNumPoints + j ) + 3 ) = 0.5 + DotProduct( delta, axis[1] ) * _texCoordScale;
                            *( ( float* )pointBuffer + 5 * ( oldNumPoints + j ) + 4 ) = 0.5 + DotProduct( delta, axis[2] ) * _texCoordScale;
                        }
                    }
