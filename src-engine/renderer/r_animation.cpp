...
//  Created:
//  Compilers:
//  Description: All bones should be an identity orientation to display the mesh exactly
//               as it is specified.
...
static int R_CullModel( mdsHeader_t* header, trRefEntity_t* ent )
{
    ...
    _frameSize = ( int )( sizeof( mdsFrame_t ) - sizeof( mdsBoneFrameCompressed_t ) + header->numBones * sizeof( mdsBoneFrameCompressed_t ) );
...
//----(SA)	end
        ...
        if( !personalModel )
        {
            ...
            R_AddDrawSurf( ( surfaceType_t* )surface, shader, fogNum, false, ATI_TESS_TRUFORM );
        }
...
__inline void LocalAngleVector( vec3_t _angles, vec3_t forward )
{
    LAVangle = _angles[YAW] * ( M_PI * 2 / 360 );
    sy = sin( LAVangle );
    cy = cos( LAVangle );
    LAVangle = _angles[PITCH] * ( M_PI * 2 / 360 );
    sp = sin( LAVangle );
    cp = cos( LAVangle );
...
__inline void LocalVectorMA( vec3_t org, float dist, vec3_t _vec, vec3_t out )
{
    out[0] = org[0] + dist * _vec[0];
    out[1] = org[1] + dist * _vec[1];
    out[2] = org[2] + dist * _vec[2];
}
...
__inline void Matrix4MultiplyInto3x3AndTranslation( /*const*/ vec4_t a[4], /*const*/ vec4_t b[4], vec3_t dst[3], vec3_t _t )
{
    dst[0][0] = a[0][0] * b[0][0] + a[0][1] * b[1][0] + a[0][2] * b[2][0] + a[0][3] * b[3][0];
    dst[0][1] = a[0][0] * b[0][1] + a[0][1] * b[1][1] + a[0][2] * b[2][1] + a[0][3] * b[3][1];
    dst[0][2] = a[0][0] * b[0][2] + a[0][1] * b[1][2] + a[0][2] * b[2][2] + a[0][3] * b[3][2];
...
__inline void Matrix4FromTranslation( const vec3_t _t, vec4_t dst[4] )
{
    int i, j;
...
// TTimo: const vec_t ** would require explicit casts for ANSI C conformance
// see unix/const-arg.c in Wolf MP source
__inline void Matrix4FromAxisPlusTranslation( /*const*/ vec3_t axis[3], const vec3_t _t, vec4_t dst[4] )
{
    int i, j;
    for( i = 0; i < 3; i++ )
    {
...
// TTimo: const vec_t ** would require explicit casts for ANSI C conformance
// see unix/const-arg.c in Wolf MP source
__inline void Matrix4FromScaledAxisPlusTranslation( /*const*/ vec3_t axis[3], const float scale, const vec3_t _t, vec4_t dst[4] )
{
    int i, j;
...
void R_CalcBones( mdsHeader_t* header, const refEntity_t* refent, int* boneList, int numBones )
{
    ...
    if( !backlerp && !torsoBacklerp )
    {
        ...
        for( i = 0; i < numBones; i++, _boneRefs++ )
        {
        
            if( validBones[*_boneRefs] )
            {
                // this bone is still in the cache
                bones[*_boneRefs] = rawBones[*_boneRefs];
                continue;
            }
            
            // find our parent, and make sure it has been calculated
            if( ( boneInfo[*_boneRefs].parent >= 0 ) && ( !validBones[boneInfo[*_boneRefs].parent] && !newBones[boneInfo[*_boneRefs].parent] ) )
            {
...
//		memcpy( pIndexes, triangles, sizeof( triangles[0] ) * indexes );
    ...
    else
    {
        ...
        for( j = 0 ; j < indexes ; j += 3 )
        {
            ...
            *( pIndexes++ ) = ( short )( baseVertex + p0 );
            *( pIndexes++ ) = ( short )( baseVertex + p1 );
            *( pIndexes++ ) = ( short )( baseVertex + p2 );
            tess.numIndexes += 3;
        }
