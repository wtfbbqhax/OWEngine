...
//
// -------------------------------------------------------------------------
//  File name:   win_input.c
//  Version:     v1.00
//  Created:
//  Compilers:
//  Description: win32 mouse and joystick code
// -------------------------------------------------------------------------
//  History:
...
void IN_GetGamepadReading( gamepadInfo_t* gamepad, int userIndex )
{
    ...
    gamepad->connected = ( getStateR == ERROR_SUCCESS ) ? true : false;
    ...
    if( triggerPressed )
    {
        ...
        gamepad->releasedLeftTrigger = false;
    }
    else
    {
    ...
    if( triggerPressed )
    {
        ...
        gamepad->releasedRightTrigger = false;
    }
    else
    {
...
void IN_ApplyGamepad( const gamepadInfo_t* gamepad )
{
    ...
    for( i = 0; i < _countof( gamepadButtonMappings ); ++i )
    {
        ...
        {
            Sys_QueEvent( g_wv.sysMsgTime, SE_KEY, button->quake, true, 0, NULL );
        }
        else if( gamepad->releasedButtons & button->xinput )
        {
            Sys_QueEvent( g_wv.sysMsgTime, SE_KEY, button->quake, false, 0, NULL );
        }
    }
    ...
    {
        Sys_QueEvent( g_wv.sysMsgTime, SE_KEY, K_GAMEPAD_RTRIGGER, true, 0, NULL );
    }
    else if( gamepad->releasedRightTrigger )
    {
        Sys_QueEvent( g_wv.sysMsgTime, SE_KEY, K_GAMEPAD_RTRIGGER, false, 0, NULL );
    }
    
    if( gamepad->pressedLeftTrigger )
    {
        Sys_QueEvent( g_wv.sysMsgTime, SE_KEY, K_GAMEPAD_LTRIGGER, true, 0, NULL );
    }
    else if( gamepad->releasedLeftTrigger )
    {
        Sys_QueEvent( g_wv.sysMsgTime, SE_KEY, K_GAMEPAD_LTRIGGER, false, 0, NULL );
    }
