...
import com.owengine.owe.OWEUtils.Button;
import com.owengine.owe.OWEUtils.Finger;
import com.owengine.owe.OWEUtils.Joystick;
import com.owengine.owe.OWEUtils.Paintable;
import com.owengine.owe.OWEUtils.Slider;
import com.owengine.owe.OWEUtils.TouchListener;
import com.owengine.owe.OWEUtils.UiElement;
...
public class OWEUiView extends GLSurfaceView implements GLSurfaceView.Renderer
{

    public OWEUiView( Context context )
    {
        super( context );
        
        OWEUtils.usegles20 = false;
    ...
    class MenuOverlay extends Paintable implements TouchListener
    {
        ...
        public MenuOverlay( int center_x, int center_y, int w, int h )
        {
            ...
            width = w;
            height = h;
            texw = OWEUtils.nextpowerof2( w );
            texh = OWEUtils.nextpowerof2( h );
        ...
        {
            tex_ind = OWEUtils.loadGLTexture( gl, texbmp );
        }
        ...
        {
            x = Math.min( Math.max( width / 2, x ), OWEUiView.this.width - width / 2 );
            y = Math.min( Math.max( height / 2 + yoffset, y ), OWEUiView.this.height + yoffset - height / 2 );
            cx = x;
            cy = y;
    ...
    public ArrayList<TouchListener> touch_elements = new ArrayList<OWEUtils.TouchListener>( 0 );
    public ArrayList<Paintable> paint_elements = new ArrayList<OWEUtils.Paintable>( 0 );
    
    public final int step = OWEUtils.dip2px( getContext(), 5 );
    ...
    public void SaveAll()
    {
        ...
        for( int i = 0; i < touch_elements.size(); i++ )
        {
            if( touch_elements.get( i ) instanceof Button )
            {
                ...
                mEdtr.putString( OWEUtils.pref_controlprefix + i, new UiElement( tmp.cx, tmp.cy, tmp.width, ( int )( tmp.alpha * 100 ) ).SaveToString() );
            }
            ...
            if( touch_elements.get( i ) instanceof Joystick )
            {
                ...
                mEdtr.putString( OWEUtils.pref_controlprefix + i, new UiElement( tmp.cx, tmp.cy, tmp.size / 2, ( int )( tmp.alpha * 100 ) ).SaveToString() );
            }
        }
        mEdtr.commit();
    }
    ...
    public boolean onTouchEvent( MotionEvent event )
    {
        ...
        for( FingerUi f: fingers )
            if( f.target != null ) UiOnTouchEvent( f, event );
