...
void SetBrushContents( brush_t* b )
{
    ...
    {
        printf( "Mixed detail and structural (defaulting to structural)", mapEnt->mapEntityNum, entitySourceBrushes, qfalse );
    ...
    for( axis = 0; axis < 3; axis++ )
    {
        for( dir = -1; dir <= 1; dir += 2, order++ )
        {
            ...
            if( i == buildBrush->numsides )
            {
                ...
                {
                    printf( "MAX_BUILD_SIDES %d\n", buildBrush->entityNum, buildBrush->brushNum, qtrue );
                    exit( 0 );
                }
                memset( s, 0, sizeof( *s ) );
                buildBrush->numsides++;
...
static void ParseRawBrush( qboolean onlyLights )
{
    ...
    while( 1 )
    {
        ...
        if( buildBrush->numsides >= MAX_BUILD_SIDES )
        {
            printf( "MAX_BUILD_SIDES %d %d", buildBrush->entityNum, buildBrush->brushNum );
            exit( 0 );
        }
...
qboolean RemoveDuplicateBrushPlanes( brush_t* b )
{
    ...
    for( i = 1 ; i < b->numsides ; i++ )
    {
        ...
        {
            printf( "degenerate plane %d %d", b->entityNum, b->brushNum, qfalse );
            // remove it
        ...
        for( j = 0 ; j < i ; j++ )
        {
            ...
            {
                printf( "duplicate plane %d %d", b->entityNum, b->brushNum, qfalse );
            ...
            if( sides[i].planenum == ( sides[j].planenum ^ 1 ) )
            {
                ...
                printf( "mirrored plane %d %d", b->entityNum, b->brushNum, qfalse );
                return qfalse;
            }
        }
    }
