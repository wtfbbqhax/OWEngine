...
import com.owengine.owe.OWEUtils.Finger;
import com.owengine.owe.OWEUtils.Paintable;
import com.owengine.owe.OWEUtils.TouchListener;
import com.owengine.owe.OWEUtils.UiLoader;
...
class OWEView extends GLSurfaceView implements GLSurfaceView.Renderer
{
    ...
    public class OWEConfigChooser implements EGLConfigChooser
    {
        int r;
        int g;
        int b;
        ...
        public OWEConfigChooser( int inr, int ing, int inb, int ina, int inmsaa, boolean gles2 )
        {
            r = inr;
            g = ing;
            b = inb;
    ...
    public OWEView( Context context )
    {
        super( context );
        
        mHandler = new Handler();
        
        OWEUtils.usegles20 = OWEUtils.owei.isD3 || OWEUtils.owei.isQ1;
        int msaa = PreferenceManager.getDefaultSharedPreferences( this.getContext() ).getInt( OWEUtils.pref_msaa, 0 );
        switch( msaa )
        {
            case 0:
                msaa = 0;
        ...
        if( PreferenceManager.getDefaultSharedPreferences( this.getContext() ).getBoolean( OWEUtils.pref_32bit, false ) )
        {
            setEGLConfigChooser( new OWEConfigChooser( 8, 8, 8, 8, msaa, OWEUtils.usegles20 ) );
            getHolder().setFormat( PixelFormat.RGBA_8888 );
        }
        else
        {
            //if (OWEUtils.owei.isD3)
            setEGLConfigChooser( new OWEConfigChooser( 5, 6, 5, 0, msaa, OWEUtils.usegles20 ) );
            getHolder().setFormat( PixelFormat.RGB_565 );
        }
        
        if( OWEUtils.usegles20 )
    ...
    //RTCW4A-specific
    OWEUtils.Button actbutton;
    OWEUtils.Button kickbutton;
        ...
        public void run()
        {
            try
            {
                ...
                Runnable qevnt_runnable = new Runnable()
                {
                    ...
                    public void run()
                    {
                        OWEJNI.sendMotionEvent( readmouse_dx, readmouse_dy );
                        readmouse_dx = 0;
                        readmouse_dy = 0;
                ...
                Runnable qkeyevnt_runnable = new Runnable()
                {
                    ...
                    public void run()
                    {
                        OWEJNI.sendKeyEvent( readmouse_keystate, readmouse_keycode, 0 );
                    }
                };
                while( fis.read( arr, 0, sizeofstruct ) != -1 )
                {
                    if( !OWECallbackObj.reqThreadrunning )
                    {
                        Thread.yield();
                        continue;
                    }
    ...
    public void onSurfaceChanged( GL10 gl, int w, int h )
    {
        if( mInit == false )
        {
            ...
            hideonscr = mPrefs.getBoolean( OWEUtils.pref_hideonscr, false );
            mapvol = mPrefs.getBoolean( OWEUtils.pref_mapvol, false );
            boolean detectMouse = mPrefs.getBoolean( OWEUtils.pref_detectmouse, true );
            
            mouse_name = hideonscr ? ( detectMouse ? detectmouse() : mPrefs.getString( OWEUtils.pref_eventdev, "/dev/input/event???" ) ) : null;
            mouse_corner = mPrefs.getInt( OWEUtils.pref_mousepos, 3 );
            
            orig_width = w;
            orig_height = h;
            
            switch( mPrefs.getInt( OWEUtils.pref_scrres, 0 ) )
            {
                case 0:
                    width = w;
                    height = h;
                ...
                case 3:
                    ...
                    break;
                case 4:
                    width = Integer.parseInt( mPrefs.getString( OWEUtils.pref_resx, "640" ) );
                    height = Integer.parseInt( mPrefs.getString( OWEUtils.pref_resy, "480" ) );
                    break;
            }
            
            OWEJNI.init( lib_dir + "/" + OWEUtils.owei.libname, width, height, OWEMain.datadir, OWEMain.datadir + "/" + mPrefs.getString( OWEUtils.pref_params, OWEUtils.owei.libname ) );
            
            UiLoader uildr = new UiLoader( this, gl, orig_width, orig_height );
            
            for( int i = 0; i < OWEUtils.owei.UI_SIZE; i++ )
            {
            ...
            if( OWEUtils.owei.isRTCW )
            {
                actbutton = ( OWEUtils.Button )touch_elements.get( OWEUtils.owei.RTCW4A_UI_ACTION );
                kickbutton = ( OWEUtils.Button )touch_elements.get( OWEUtils.owei.RTCW4A_UI_KICK );
            }
            else
            {
                actbutton = null;
            ...
            //must be last
            touch_elements.add( new OWEUtils.MouseControl( this ) );
            touch_elements.add( new OWEUtils.MouseControl( this ) );
            touch_elements.add( new OWEUtils.MouseControl( this ) );
    ...
    public boolean onKeyDown( int keyCode, KeyEvent event )
    {
        ...
        int qKeyCode = OWEKeyCodes.convertKeyCode( keyCode, event );
    ...
    public static ArrayList<TouchListener> touch_elements = new ArrayList<OWEUtils.TouchListener>( 0 );
    public static ArrayList<Paintable> paint_elements = new ArrayList<OWEUtils.Paintable>( 0 );
    ...
    public boolean sendKeyEvent( final boolean down, final int keycode, final int charcode )
    {
        queueEvent( new Runnable()
        {
            public void run()
            {
                OWEJNI.sendKeyEvent( down ? 1 : 0, keycode, charcode );
            }
        } );
        return true;
    }
    ...
    public boolean sendMotionEvent( final float deltax, final float deltay )
    {
        queueEvent( new Runnable()
        {
            public void run()
            {
                OWEJNI.sendMotionEvent( deltax, deltay );
            }
        } );
        return true;
    }
    ...
    private boolean sendTrackballEvent( final boolean down, final float x, final float y )
    {
        queueEvent( new Runnable()
        {
            public void run()
            {
                ...
                OWEJNI.sendMotionEvent( x - last_trackball_x, y - last_trackball_y );
                last_trackball_x = x;
                last_trackball_y = y;
            }
        } );
